// طبقة الفواتير (واجهة): قائمة + عرض فاتورة + تعديل مباشر
...
export function getView() {
  return current ? 'invoice' : 'list';
}

export function getCurrentInvoiceId() {
  return current?.invoice?.id || null;
}
...
function rowAtPos(p) {
  return current?.rows?.find(r => Number(r.position) === Number(p));
}
    ...
    return `
      <div class="inv-card ${cfg.theme}" data-open="${inv.id}">
        ...
        <div class="body">
          ...
          <div class="stats">
            <span>عدد البنود: <b>${inv.count ?? 0}</b></span>
            <span>الإجمالي: <b>${fmtMoney(inv.total ?? 0)}</b></span>
          </div>
        </div>
      </div>`;
  }).join('');
...
function renderTable(inv, cfg) {
  ...
  h += `</tbody></table></div>
    <button type="button" class="add-row-btn" data-action="add-row">+ إضافة بند جديد</button>`;
  return h;
}
...
async function saveLetter() {
  const m = letterMetaFromDom();
  try {
    await saveInvoiceMeta(m);
  } catch {
    toast('تعذّر الحفظ');
  }
}
